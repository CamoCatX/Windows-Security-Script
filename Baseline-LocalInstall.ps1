...
the corresponding baseline:
 -Win11DomainJoined    - Windows 11, domain-joined
 -Win11NonDomainJoined - Windows 11, non-domain-joined
...
.PARAMETER Win11DomainJoined
Installs security configuration baseline for Windows 11, domain-joined

.PARAMETER Win11NonDomainJoined
Installs security configuration baseline for Windows 11, non-domain-joined
...
param(
    [Parameter(Mandatory = $true, ParameterSetName = 'Win11DJ')]
    [switch]
    $Win11DomainJoined,

    [Parameter(Mandatory = $true, ParameterSetName = 'Win11NonDJ')]
    [switch]
    $Win11NonDomainJoined
...
$GPO_Win11_BitLocker = "MSFT Windows 11 - BitLocker"
$GPO_Win11_Computer  = "MSFT Windows 11 - Computer"
$GPO_Win11_CredentialGuard = "MSFT Windows 11 - Credential Guard"
$GPO_Win11_DefenderAV  = "MSFT Windows 11 - Defender Antivirus"
$GPO_Win11_DomainSec   = "MSFT Windows 11 - Domain Security"
$GPO_Win11_User      = "MSFT Windows 11 - User"
...
# GPOs for Windows 11
if ($Win11DomainJoined -or $Win11NonDomainJoined)
{
    if ($Win11DomainJoined)
    {
        $baselineLabel = "Windows 11 - domain-joined"
    }
    else
    {
        $baselineLabel = "Windows 11 - non-domain-joined"
    }
    AddToCollection $GPOs $GPO_IE11_Computer
    AddToCollection $GPOs $GPO_IE11_User
    AddToCollection $GPOs $GPO_Win11_Computer
    AddToCollection $GPOs $GPO_Win11_User
    AddToCollection $GPOs $GPO_Win11_BitLocker
    AddToCollection $GPOs $GPO_Win11_DomainSec
    AddToCollection $GPOs $GPO_Win11_DefenderAV
    AddToCollection $GPOs $GPO_Win11_CredentialGuard
}
...
Copy-Item -Force ..\Templates\en-US\*.adml $env:windir\PolicyDefinitions\en-US
Log $dline
...
if ($Win11DomainJoined -or $Win11NonDomainJoined)
{
    LogAndShowProgress "Disable Xbox scheduled task on Win11..."
...
if ($Win11NonDomainJoined -or $WSNonDomainJoined)
{
